...
(define (test-mh samples lag runs model expectation-fn true-expectation tolerance . message)
 (let ((expr (case query-type
               ((hmc)  `(hmc-query ',tempering-vars ',tempering-temps ,samples ,lag ,@model) )
               ((gradient)  `(gradient-query ',tempering-vars ',tempering-temps ,samples ,lag ,@model) )
               ((pt)  `(pt-query ',tempering-vars ',tempering-temps ,samples ,lag ,@model) )
               ;((mtm) `(mtm-lex-query ,samples ,(* samples-before-swap swaps) 4 ,@lex-query-triple (get-current-environment) ) )
               ((tt) `(tt-query ',tempering-vars ',TT-base-temps ',TT-temps ,samples ,lag ,@model) )
               ((mh) `(mh-query ,samples ,lag ,@model ) ))))
   (for-each display 
             `("\n\nM-H Test: " ,@message
               "   query: " ,query-type
               "   samples: " ,samples 
...
;;        ))
   ...
   (let* ((mh-proc (lambda () (scored-value->value (church-top-level-eval (list '(define noise 0) expr)))))
